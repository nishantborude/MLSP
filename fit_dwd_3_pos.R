...
trainFNC <- read.csv("~/MLSP/Train/train_FNC.csv", as.is=T, header=T, sep=",")
trainSBM <- read.csv("~/MLSP/Train/train_SBM.csv", as.is=T, header=T, sep=",")
trainLAB <- read.csv("~/MLSP/Train/train_labels.csv", as.is=T, header=T, sep=",")

# READ TEST

testFNC <- read.csv("~/MLSP/Test/test_FNC.csv", as.is=T, header=T, sep=",")
testSBM <- read.csv("~/MLSP/Test/test_SBM.csv", as.is=T, header=T, sep=",")


# SUBMISSION EXAMPLE

myExample <- read.csv("~/MLSP/submission_example.csv", as.is=T, header=T, sep=",")
...
# TEST
data = merge(testFNC, testSBM, by = "Id")

rm(trainFNC, trainSBM, trainLAB, testFNC, testSBM)


myTest <- t(data[,-1])
colnames(myTest) <- myExample$Id
...
write.csv(myExample, file="~/MLSP/results/dwd_fit.csv", row.names=F)
